    ...
    private void ApplyMarkings(Entity<VisualOrganMarkingsComponent> ent, Entity<SpriteComponent?> target)
    {
        ...
        foreach (var marking in AllMarkings(ent))
        {
            ...
            for (var i = 0; i < proto.Sprites.Count; i++)
            {
                ...
                if (proto.Shader != null && TryComp<SpriteComponent>(target, out var spriteComponent)) // Aurora's Song - Get sprite component
                {
                    // use spriteComponent's layersetshader function to set the layer's shader to that which is specified.
                    spriteComponent.LayerSetShader(layerId, proto.Shader); // Switch sprite component names
                }
                // impstation edit end
