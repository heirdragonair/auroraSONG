...
public sealed partial class CartridgeLoaderSystem
{
    ...
    public void ActivateProgram(Entity<CartridgeLoaderComponent> ent, EntityUid program)
    {
        ...
        // Imp Start: Logic for raising the active program changed event
        var ev = new ActiveProgramChangedEvent(ent, ent.Comp.ActiveProgram, program);
    ...
    public void DeactivateProgram(Entity<CartridgeLoaderComponent> ent, EntityUid program)
    {
        ...
        // Imp Start: Logic for raising the active program changed event
        var ev = new ActiveProgramChangedEvent(ent, program, ent.Comp.ActiveProgram);
        RaiseLocalEvent(ent, ref ev);
        // Imp End
        Dirty(ent);
        UpdateUiState(ent.AsNullable());
    ...
    {
        if (!TryComp<CartridgeComponent>(cartridge, out var cartridgeComponent)) // Aurora's Song - Make TryComp
            return false;
        ...
        if (cartridgeComponent.Disposable) // Frontier: Delete the cartridge after install if its disposable.
            QueueDel(ent.Comp.CartridgeSlot.ContainerSlot!.ContainedEntity); // Frontier
