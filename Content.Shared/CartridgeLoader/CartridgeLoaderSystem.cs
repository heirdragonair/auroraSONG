...
public sealed partial class CartridgeLoaderSystem : EntitySystem
{
    ...
    private void OnItemInserted(Entity<CartridgeLoaderComponent> ent, ref EntInsertedIntoContainerMessage args)
    {
        ...
        if (TryComp<CartridgeComponent>(args.Entity, out var cartridge))
        {
            ...
            else if (args.Container.ID == CartridgeLoaderComponent.UnremovableContainerId || cartridge.Readonly) // Aurora's Song - Add readonly flag
        ...
        if (cartridge != null && cartridge.AutoInstall)
            InstallCartridge(ent, args.Entity); // Aurora's Song - Use new syntax
        // End Frontier
    }
...
/// <summary>
/// Imp: Raised whenever the currently open program changes
/// </summary>
[ByRefEvent]
