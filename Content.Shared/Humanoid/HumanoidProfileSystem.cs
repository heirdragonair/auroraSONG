using Content.Shared._AS.Traits; // Aurora's Song
...
public sealed class HumanoidProfileSystem : EntitySystem
{
    ...
    private void OnExamined(Entity<HumanoidProfileComponent> ent, ref ExaminedEvent args)
    {
        ...
        if (HasComp<ReplicantComponent>(ent)) // Aurora's Song - Replika
        {
            species = Loc.GetString("species-name-replicant", ("species", species)).ToLower();
        }
