...
public sealed class MarkingManager
{
    ...
    private bool CanBeApplied(MarkingsGroupPrototype group, Sex sex, MarkingPrototype prototype, bool whitelisted)
    {
        ...
        {
            if (!prototype.GroupWhitelist.Contains(group) != prototype.InvertGroupWhitelist) // DEN - Invert group whitelists
                return false;
        }

        return prototype.SexRestriction == null || prototype.SexRestriction == sex != prototype.InvertSexRestriction; // DEN - Invert sex whitelists
    }
